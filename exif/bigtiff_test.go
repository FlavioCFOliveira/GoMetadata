...
//
// The fixture is embedded in exif/testdata/BigTIFF_LE.tif (4744 bytes).
...
// The fixture is embedded in exif/testdata/BigTIFF_BE.tif (4744 bytes).
func TestParseBigTIFF_RealFileBE(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile("testdata/BigTIFF_BE.tif")
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
	e, ferr := Parse(data)
