...
func parseAt(b []byte, offset int) map[uint16][]byte {
	...
	for i := 0; i < count; i++ { //nolint:intrange,modernize // binary parser: loop variable is a byte-slice offset multiplier
		tag, value, ok := parseSigmaIFDEntry(b, offset+2+i*12)
		if !ok {
			continue
		}
		result[tag] = value
...
// parseSigmaIFDEntry decodes a single 12-byte IFD entry at pos within b
// using little-endian byte order (Sigma MakerNote is always LE).
...
func parseSigmaIFDEntry(b []byte, pos int) (tag uint16, value []byte, ok bool) {
