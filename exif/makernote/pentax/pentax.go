...
func parseIFDAt(b []byte, offset int, bigEndian bool) map[uint16][]byte {
	...
	for i := 0; i < count; i++ {
		tag, value, ok := parsePentaxIFDEntry(b, offset+2+i*12, bigEndian)
		if !ok {
			continue
		}
		result[tag] = value
...
// parsePentaxIFDEntry decodes a single 12-byte IFD entry at pos within b.
...
func parsePentaxIFDEntry(b []byte, pos int, bigEndian bool) (tag uint16, value []byte, ok bool) {
