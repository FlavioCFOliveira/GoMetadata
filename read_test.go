...
// TestMetadata_GPSXMPFallback covers the W3C Geo namespace fallback path in
// Metadata.GPS(). GPS coordinates stored under the W3C Basic Geo vocabulary
// (http://www.w3.org/2003/01/geo/wgs84_pos#, properties lat and lon/long) must
// be decoded when the Adobe XMP exif:GPSLatitude / exif:GPSLongitude properties
// are absent.
func TestMetadata_GPSXMPFallback(t *testing.T) {
	t.Parallel()
	// W3C Geo namespace — lat="48,51.35N" lon="2,21.07E"
	...
	lat, lon, ok := m.GPS()
	if !ok {
		t.Fatal("GPS() returned ok=false for W3C Geo namespace coordinates; xmp.GPS() must decode the fallback namespace")
	}
...
// TestReadBigTIFF verifies end-to-end BigTIFF EXIF decoding through the public
// API, using metadata-rich fixtures generated from ExifTool.tif.
//
// This test was rewritten as part of task #54's second pass to provide HARD
// assertions on EXIF tag values. The previous version gated all tag assertions
// behind t.Log-and-continue, which caused it to pass even when EXIF decoding
// was completely broken. That toothless behaviour masked the root defect.
//
// Fixtures are embedded in testdata/fixtures/ (committed, ~4.7 KB each).
// They were originally generated with:
//
//	tiffcp -8 -L testdata/corpus/tiff/exiftool/ExifTool.tif BigTIFF_LE.tif
//	tiffcp -8 -B testdata/corpus/tiff/exiftool/ExifTool.tif BigTIFF_BE.tif
//
// Expected tags (from tiffinfo/exiftool on the fixture files):
//
//	Make:             Canon
//	Model:            Canon EOS DIGITAL REBEL
//	ImageDescription: The picture caption
//	Software:         GraphicConverter
//	ImageWidth:       160
//	ImageHeight:      120
//
// The same values must decode from BOTH the LE and BE BigTIFF variants.
// Failure on any tag is a hard test failure — t.Log-and-continue is forbidden.
func TestReadBigTIFF(t *testing.T) {
	t.Parallel()

	const (
		fixtureLE = "testdata/fixtures/BigTIFF_LE.tif"
		fixtureBE = "testdata/fixtures/BigTIFF_BE.tif"

		// Expected values — must match both LE and BE variants.
		// Source: exiftool / tiffinfo on BigTIFF_LE.tif and BigTIFF_BE.tif.
		wantMake    = "Canon"
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("ReadFile(%q): %v", tc.fixture, err)
			}
			if m == nil {
				t.Fatal("ReadFile returned nil Metadata")
			}

			// Format must be FormatTIFF.
			if got := m.Format(); got != format.FormatTIFF {
				t.Errorf("Format() = %v, want FormatTIFF", got)
			}

			// RawEXIF must be non-nil (the whole BigTIFF stream is the EXIF payload).
			if m.RawEXIF() == nil {
				t.Fatal("RawEXIF() is nil; expected BigTIFF byte stream")
			}

			// EXIF must be populated — if it is nil, all tag assertions below will
			// fail, but we want to report that EXIF parsing itself failed.
			if m.EXIF == nil {
				t.Fatal("EXIF is nil; BigTIFF IFD traversal produced no result")
			}

			// --- Hard tag assertions (t.Fatal/t.Error, never t.Log) ---

			// Make (IFD0 tag 0x010F, EXIF §4.6.4).
			if got := m.Make(); got != wantMake {
				t.Errorf("Make() = %q, want %q", got, wantMake)
			}

			// Model (IFD0 tag 0x0110, EXIF §4.6.4).
			if got := m.CameraModel(); got != wantModel {
				t.Errorf("CameraModel() = %q, want %q", got, wantModel)
			}

			// ImageDescription / Caption (IFD0 tag 0x010E, EXIF §4.6.4).
			if got := m.Caption(); got != wantCaption {
				t.Errorf("Caption() = %q, want %q", got, wantCaption)
			}

			// ImageWidth and ImageHeight.
			// In BigTIFF these are TypeShort (3) stored inline in the 8-byte
			// value-or-offset field.  The ImageSize() accessor reads PixelXDimension
			// (0xA002) and PixelYDimension (0xA003) from ExifIFD; this file carries
			// ImageWidth (0x0100) and ImageLength (0x0101) in IFD0 instead.
			// Use the raw EXIF IFD0 entries directly.
			if m.EXIF.IFD0 != nil {
				widthEntry := m.EXIF.IFD0.Get(exif.TagImageWidth)
				heightEntry := m.EXIF.IFD0.Get(exif.TagImageLength)
				if widthEntry == nil {
					t.Error("IFD0 ImageWidth (0x0100) entry missing")
				} else {
					var w uint32
					switch widthEntry.Type {
					case exif.TypeShort:
						w = uint32(widthEntry.Uint16())
					default:
						w = widthEntry.Uint32()
					}
					if w != wantWidth {
						t.Errorf("ImageWidth = %d, want %d", w, wantWidth)
					}
				}
				if heightEntry == nil {
					t.Error("IFD0 ImageLength (0x0101) entry missing")
				} else {
					var h uint32
					switch heightEntry.Type {
					case exif.TypeShort:
						h = uint32(heightEntry.Uint16())
					default:
						h = heightEntry.Uint32()
					}
					if h != wantHeight {
						t.Errorf("ImageLength = %d, want %d", h, wantHeight)
					}
				}
			}
		})
