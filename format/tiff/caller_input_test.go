...
func injectORF(in []byte, w io.Writer) error {
	return InjectWithEXIFORFStream(bytes.NewReader(in), in, true, nil, nil, []byte(testXMPPacket), w)
}

func injectRW2(in []byte, w io.Writer) error {
	return InjectWithEXIFRW2Stream(bytes.NewReader(in), in, true, nil, nil, []byte(testXMPPacket), w)
}
