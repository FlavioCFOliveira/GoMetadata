...
func TestRW2RoundTripThumbnailOffset(t *testing.T) {
	...
	if err := InjectWithEXIFRW2Stream(bytes.NewReader(rw2), rw2, true, nil, nil, []byte("<x:xmpmeta/>"), &out); err != nil {
...
func TestTIFFInjectRejectsWireFrameXMP(t *testing.T) {
	...
	t.Run("InjectWithEXIF", func(t *testing.T) {
		...
		err := InjectWithEXIFStream(bytes.NewReader(tiffData), tiffData, true, e, nil, rawWireXMP, io.Discard)
		if err == nil {
			t.Error("accepted wire-frame XMP without error (task #118 regression)")
			return
		}
	...
	t.Run("InjectWithEXIFCR2", func(t *testing.T) {
		...
		tiffData := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFCR2Stream(bytes.NewReader(tiffData), tiffData, true, e, nil, rawWireXMP, io.Discard)
		if err == nil {
			t.Error("accepted wire-frame XMP without error (task #118 regression)")
			return
		}
	...
	t.Run("InjectWithEXIFNEF", func(t *testing.T) {
		...
		tiffData := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFNEFStream(bytes.NewReader(tiffData), tiffData, true, e, nil, rawWireXMP, io.Discard)
		if err == nil {
			t.Error("accepted wire-frame XMP without error (task #118 regression)")
			return
		}
	...
	t.Run("InjectWithEXIFARW", func(t *testing.T) {
		...
		tiffData := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFARWStream(bytes.NewReader(tiffData), tiffData, true, e, nil, rawWireXMP, io.Discard)
		if err == nil {
			t.Error("accepted wire-frame XMP without error (task #118 regression)")
			return
		}
	...
	t.Run("InjectWithEXIFORF", func(t *testing.T) {
		...
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFORFStream(bytes.NewReader(orfBytes), orfBytes, true, e, nil, rawWireXMP, io.Discard)
		if err == nil {
			t.Error("accepted wire-frame XMP without error (task #118 regression)")
			return
		}
	...
	t.Run("InjectWithEXIFRW2", func(t *testing.T) {
		...
		rw2 := buildRW2WithIFD1()
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFRW2Stream(bytes.NewReader(rw2), rw2, true, e, nil, rawWireXMP, io.Discard)
		if err == nil {
			t.Error("accepted wire-frame XMP without error (task #118 regression)")
			return
		}
...
func TestTIFFInjectNilXMPNotRejected(t *testing.T) {
	...
	t.Run("InjectWithEXIF", func(t *testing.T) {
		...
		data := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFStream(bytes.NewReader(data), data, true, e, nil, nil, io.Discard)
		if err != nil && errors.Is(err, ErrCorruptXMP) {
	...
	t.Run("InjectWithEXIFCR2", func(t *testing.T) {
		...
		data := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFCR2Stream(bytes.NewReader(data), data, true, e, nil, nil, io.Discard)
		if err != nil && errors.Is(err, ErrCorruptXMP) {
	...
	t.Run("InjectWithEXIFNEF", func(t *testing.T) {
		...
		data := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFNEFStream(bytes.NewReader(data), data, true, e, nil, nil, io.Discard)
		if err != nil && errors.Is(err, ErrCorruptXMP) {
	...
	t.Run("InjectWithEXIFARW", func(t *testing.T) {
		...
		data := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFARWStream(bytes.NewReader(data), data, true, e, nil, nil, io.Discard)
		if err != nil && errors.Is(err, ErrCorruptXMP) {
	...
	t.Run("InjectWithEXIFORF", func(t *testing.T) {
		...
		data := buildMinimalTIFF(binary.LittleEndian, nil, nil)
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFORFStream(bytes.NewReader(data), data, true, e, nil, nil, io.Discard)
		if err != nil && errors.Is(err, ErrCorruptXMP) {
	...
	t.Run("InjectWithEXIFRW2", func(t *testing.T) {
		...
		rw2 := buildRW2WithIFD1()
		e := &exif.EXIF{IFD0: &exif.IFD{}}
		err := InjectWithEXIFRW2Stream(bytes.NewReader(rw2), rw2, true, e, nil, nil, io.Discard)
		if err != nil && errors.Is(err, ErrCorruptXMP) {
...
func TestTIFFWritePathPreservesIPTCOnXMPOnlyUpdate(t *testing.T) {
	...
	var out bytes.Buffer
	if err := InjectWithEXIFStream(bytes.NewReader(tiffData), tiffData, true, nil, rawIPTC, []byte("<x:xmpmeta/>"), &out); err != nil {
...
func TestTIFFInjectEmptyIPTCNoZeroLengthTag(t *testing.T) {
	...
	t.Run("relocate-empty-rawIPTC-no-tag", func(t *testing.T) {
		...
		var out bytes.Buffer
		if err := InjectWithEXIFStream(bytes.NewReader(tiffData), tiffData, true, nil, emptyIPTC, nil, &out); err != nil {
