...
func TestInjectWithEXIFRealFile_CrampsTIF(t *testing.T) {
	...
	if err := InjectWithEXIFStream(bytes.NewReader(original), original, true, e, nil, nil, &out); err != nil {
...
func TestInjectWithEXIFRealFile_PassThrough(t *testing.T) {
	...
	var out bytes.Buffer
	if err := InjectWithEXIFStream(bytes.NewReader(original), original, true, nil, nil, nil, &out); err != nil {
