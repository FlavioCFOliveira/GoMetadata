...
func TestConformance_CR2_write_preserves_CR_marker(t *testing.T) {
	...
	var out bytes.Buffer
	if err := tiff.InjectWithEXIFCR2(bytes.NewReader(original), original, true, e, nil, nil, &out); err != nil {
...
func TestConformance_CR2_makernote_write_preserves_blob(t *testing.T) {
	...
	var out bytes.Buffer
	if err := tiff.InjectWithEXIFCR2(bytes.NewReader(data), data, true, e, nil, nil, &out); err != nil {
...
func TestConformance_CR2_corpus_write_preserves_CR_marker(t *testing.T) {
	...
	for _, path := range paths {
		...
		t.Run(name, func(t *testing.T) {
			...
			var out bytes.Buffer
			if writeErr := tiff.InjectWithEXIFCR2(bytes.NewReader(original), original, true, e, nil, nil, &out); writeErr != nil {
...
func TestConformance_CR2_corpus_write_strip_data_preserved(t *testing.T) {
	...
	for _, path := range paths {
		...
		t.Run(name, func(t *testing.T) {
			...
			var out bytes.Buffer
			if writeErr := tiff.InjectWithEXIFCR2(bytes.NewReader(original), original, true, eOrig, nil, nil, &out); writeErr != nil {
				t.Skipf("CR2-corpus: %s: InjectWithEXIFCR2: %v", name, writeErr)
			}
			result := out.Bytes()
