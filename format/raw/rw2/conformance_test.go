...
func TestConformance_RW2_GUID_preserved_on_write(t *testing.T) {
	...
	var out bytes.Buffer
	if err := tiff.InjectWithEXIFRW2Stream(bytes.NewReader(base), base, true, nil, nil, rawXMP, &out); err != nil {
...
func TestConformance_RW2_GUID_IFD0_offset_24_after_write(t *testing.T) {
	...
	var out bytes.Buffer
	if err := tiff.InjectWithEXIFRW2Stream(bytes.NewReader(base), base, true, nil, nil, []byte(`<xmpmeta/>`), &out); err != nil {
...
func TestConformance_RW2_recursive_IFD_rebase(t *testing.T) {
	...
	var out bytes.Buffer
	if err := tiff.InjectWithEXIFRW2Stream(bytes.NewReader(base), base, true, nil, wantIPTC, wantXMP, &out); err != nil {
...
func TestConformance_RW2_OOL_offsets_shift_by_16(t *testing.T) {
	...
	if err := tiff.InjectWithEXIFRW2Stream(bytes.NewReader(base), base, true, nil, nil, wantXMP, &outBuf); err != nil {
...
func TestConformance_RW2_corpus_write_round_trip(t *testing.T) {
	...
	for _, p := range rw2Paths {
		t.Run(filepath.Base(p), func(t *testing.T) {
			...
			var out bytes.Buffer
			if err := tiff.InjectWithEXIFRW2Stream(bytes.NewReader(data), data, true, nil, nil, nil, &out); err != nil {
