...
// FIX 5 applied to format/raw/orf/orf.go's extractTIFFTags.
...
// public Extract entry point (IIRO magic variant), confirming the whole ORF
// read path tolerates an out-of-range IFD0 offset without panicking.
func TestExtractHighIFD0OffsetEndToEnd(t *testing.T) {
	t.Parallel()

	// Minimal ORF header: "IIRO" magic (bytes [0:4]) + an out-of-range IFD0
	// offset at bytes [4:8].
	data := make([]byte, 8)
	data[0], data[1], data[2], data[3] = 0x49, 0x49, 0x52, 0x4F // "IIRO"
	...
	if rawEXIF == nil {
		t.Error("Extract() rawEXIF = nil, want the original ORF bytes preserved")
	}
