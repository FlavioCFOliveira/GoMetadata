...
//
// Gate for #140 (orf.Extract uncapped io.ReadAll).
//
...
//
// Gate for #140 (orf.Inject uncapped io.ReadAll).
//
//nolint:paralleltest // sets package-level maxFileSize; must not run in parallel
...
// TestExtractFileTooLargePositiveControl verifies that a valid ORF input
...
func TestExtractFileTooLargePositiveControl(t *testing.T) {
	...
	// buildORF() is defined in orf_test.go (same package) and produces a
	// 14-byte minimal ORF — well under capBytesOOM.
	data := buildORF()
	_, _, _, err := Extract(bytes.NewReader(data))
	if errors.Is(err, ErrFileTooLarge) {
		t.Fatalf("Extract positive control: unexpected ErrFileTooLarge for a small ORF input")
	}
}
