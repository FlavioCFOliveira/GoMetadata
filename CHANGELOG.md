...
- **CR2 metadata write via copy-and-relocate** (`write.go`, `format/format.go`, task #95): `format.SupportsWrite(FormatCR2)` now returns `true`. Canon CR2 uses standard LE TIFF magic (`II*\0`) and routes through the same `writeTIFF` copy-and-relocate path as `FormatTIFF`/`FormatDNG`. Canon MakerNote blobs are copied verbatim — per SPIKE #24, Canon uses blob-relative (self-relative) offsets, so verbatim copy is safe and correct. Validated against a real Canon EOS 350D corpus file: `ImageDataHash` IN==OUT (image pixels identical), all MakerNote and SubIFD tags preserved, only expected offset fields (`PreviewImageStart`, `ThumbnailOffset`, `StripOffsets`) changed as a result of relocation. NEF and ARW write was evaluated but remains gated (empirical validation found MakerNote data loss and SubIFD OOL RATIONAL value corruption for both — details in the Changed section below).

### Changed

- **NEF and ARW write gated after empirical validation failure** (`write.go`, `format/format.go`, task #95): `SupportsWrite(FormatNEF)` and `SupportsWrite(FormatARW)` remain `false`. Real-corpus tests against actual Nikon NEF and Sony ARW files revealed unacceptable metadata loss: NEF — SubIFD OOL RATIONAL values (XResolution/YResolution) corrupted from 72 to 1, PreviewIFD and NikonScanIFD structures lost, `ImageDataHash` mismatch (5.6 MB input → 4.9 MB output); ARW — 52 Sony MakerNote tags lost, SR2Private IFD structure corrupted, SubIFD StripOffsets wrong (917504 → 50979). Un-gating these formats is deferred to a follow-up task requiring deeper Nikon/Sony SubIFD and MakerNote investigation.

### Added (continued)

- **DNG SubIFD recursive relocation and write support** (`format/tiff/relocate.go`, task #94, epic #33): the copy-and-relocate serializer now recursively follows `SubIFDs` (tag 0x014A) from IFD0, enumerates their strip/tile image blocks, and relocates both the SubIFD structures and their image blocks alongside the main IFD blocks. This covers the canonical DNG layout (IFD0 thumbnail + SubIFDs full-res), multi-SubIFD files (0x014A count > 1, out-of-line pointer array), tiled SubIFDs, and SubIFDs with multi-strip arrays. `format.SupportsWrite(FormatDNG)` now returns `true`; `isTIFFBased()` no longer includes `FormatDNG`. New sentinel errors `tiff.ErrSubIFDPointerArrayOOB` and `tiff.ErrSubIFDEntryNotFound` are exported for diagnostic use. `exif.ParseIFDAt` is added as a thin exported wrapper around the internal `parseSingleIFD` for use by the copy-and-relocate layer. Real-corpus DNG validation is recommended before production use.

- **TIFF copy-and-relocate metadata write** (`format/tiff/`, tasks #92/#93, epic #33): `tiff.Inject` now uses a new copy-and-relocate serializer (`format/tiff/relocate.go`) that enumerates every image-data block referenced by `StripOffsets` (0x0111), `TileOffsets` (0x0144), and `JPEGInterchangeFormat` (0x0201 non-thumbnail) across IFD0 and the IFD1 chain, appends each block at a fresh absolute offset in the rebuilt TIFF stream, and patches the offset entries accordingly. Both scalar (single-strip) and array (multi-strip, multi-tile, COUNT > 1) offset entries are handled. IFD1 JPEG thumbnails continue to be relocated by `exif.Encode`'s existing `patchThumbnailEntries` path. MakerNote blobs are copied verbatim (SPIKE #24: blob-relative offsets are move-safe). New sentinel errors `tiff.ErrBlockOutOfBounds`, `tiff.ErrUnsupportedOffsetType`, `tiff.ErrTruncatedOffsetArray`, and `tiff.ErrUnsupportedElemSize` are exported for diagnostic use.
