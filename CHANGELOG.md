...
- **BigTIFF write is now fully supported through the public API** (`write.go`, `exif/write.go`, `format/tiff/relocate.go`, `format/tiff/relocate_bigtiff.go`, #264/#270/#271): `Write` and `WriteFile` no longer return `ErrWriteNotSupported` for BigTIFF sources (magic `0x002B`). `exif.Encode` gained a native BigTIFF write path (16-byte header, 20-byte IFD entries, 64-bit offsets throughout), and `format/tiff`'s copy-and-relocate serialiser became container-width-aware for image-block enumeration, `SubIFDs` (0x014A) relocation, and every raw-offset scan performed after encoding. The root-package guard (`isBigTIFFSource` in `write.go`) that pinned the old refusal behaviour has been removed now that both layers are verified end-to-end against real corpus fixtures, including LONG8 strip/tile arrays and multi-level SubIFD chains. `ErrWriteNotSupported` remains exported for future use by any format without a write path yet.
