...
//
// Error handling: if the transcoding fails (malformed input), toUTF8 returns
// nil.  Callers treat nil as "no valid UTF-8 content" and fall back to skipping
// the packet.
...
func appendUTF8Rune(dst []byte, cp uint32) []byte {
	r := rune(cp) //nolint:gosec // G115: cp is validated as a Unicode code point (≤ 0x10FFFF) before call; rune(uint32) is safe
	switch {
	case r < 0x80:
