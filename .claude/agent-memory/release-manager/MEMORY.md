...
- [v1.3.0 release record](project_v130_release.md) — RELEASED 2026-07-07 (tag v1.3.0, commit cecc682); security-gate self-verification method when Agent tool is unavailable; benchmark same-session system-load noise finding (B/op/allocs/op reliable, ns/op is not)
- [Feedback: verify gate claims, not briefings](feedback_verify_gate_claims_not_briefings.md) — a briefing's claim that security-auditor already cleared something is not evidence; cross-check agent-memory or self-verify before treating a hard gate as passed
